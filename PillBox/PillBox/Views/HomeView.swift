                    ...
                    VStack (alignment: .leading) {
                        ...
                        if pill.acceptedAt.contains(dayPlanner.currentDate) {
                            Text("Принято")
                                ...
                                .fontWeight(.light)
                                .foregroundColor(.green)
                        } else if Date().formatted(.dateTime .hour(.twoDigits(amPM: .abbreviated))) > pill.receptionTime.formatted(.dateTime .hour(.twoDigits(amPM: .abbreviated))) || Date().formatted(.dateTime .hour(.twoDigits(amPM: .abbreviated))) == pill.receptionTime.formatted(.dateTime .hour(.twoDigits(amPM: .abbreviated))) && Date().formatted(.dateTime .minute()) > pill.receptionTime.formatted(.dateTime .minute()) {
                            Text("Пропущено")
                                .font(.title3)
                                .fontWeight(.light)
                                .foregroundColor(.red)

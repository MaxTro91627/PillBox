                ...
                VStack {
                    Group {
                        ...
                        Divider()
                            ...
                            .padding(.bottom)
                        HStack {
                            DatePicker("Дата рождения", selection: $userBirthdayPicker, displayedComponents: .date)
            ...
            ForEach(dayPlanner.pills, id:\.self) { pill in
                ...
                HStack {
                    ...
                    VStack(alignment: .trailing) {
                        switch selectedStatisticMode {
                        case 0:
                            let pillCount = dayPlanner.countStatisticInRange(from: dayPlanner.setStartOfDay(for: Calendar.current.date(byAdding: .day, value: -7, to: dayPlanner.getToday())!), to: dayPlanner.getToday(), for: pill)
                            HStack {
                                Text("Назначено: ")
                        ...
                        case 1:
                            let pillCount = dayPlanner.countStatisticInRange(from: dayPlanner.setStartOfDay(for: Calendar.current.date(byAdding: .month, value: -1, to: dayPlanner.getToday())!), to: dayPlanner.getToday(), for: pill)
                            HStack {
                                Text("Назначено: ")
                                Text("\(Int(pillCount[1]))")
                                    .fontWeight(.bold)
                        ...
                        case 2:
                            let pillCount = dayPlanner.countStatisticInRange(from: dayPlanner.setStartOfDay(for: Calendar.current.date(byAdding: .year, value: -1, to: dayPlanner.getToday())!), to: dayPlanner.getToday(), for: pill)
                            HStack {
                                Text("Назначено: ")
                                Text("\(Int(pillCount[1]))")
                                    .fontWeight(.bold)
                        ...
                        case 3:
                            let pillCount = dayPlanner.countStatisticInRange(from: dayPlanner.setStartOfDay(for: startDate), to: dayPlanner.setStartOfDay( for: endDate), for: pill)
                            HStack {
                                Text("Назначено: ")
                                Text("\(Int(pillCount[1]))")
                                    .fontWeight(.bold)

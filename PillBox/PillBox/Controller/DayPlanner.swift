...
class DayPlanner: ObservableObject {
    ...
    func countStatisticInRange(from start: Date, to end: Date, for pill: Pill) -> [Float] {
        let countOfSettedPills: Int = pill.receptionDates.filter({ $0 >= start && $0 <= end}).count
        let countOfAcceptedPills: Int = pill.receptionDates.filter({ $0 >= start && $0 <= end && pill.acceptedAt.contains($0)}).count
